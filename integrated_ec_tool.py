...
class IntegratedECTool(QMainWindow):
        ...
        try:
            ...
            if self.master_hwnd:
                try:
                    ...
                    # ウィンドウの状態を確認
                    is_visible = win32gui.IsWindowVisible(self.master_hwnd)
                    is_enabled = win32gui.IsWindowEnabled(self.master_hwnd)
                    print(f"ウィンドウ状態 - Visible: {is_visible}, Enabled: {is_enabled}")
                    
                    if not is_visible:
                        print("ウィンドウが非表示状態のため、埋め込みをスキップ")
                        return
                    ...
                    # WinFormsアプリの場合、SetParentを使わずに位置同期方式を採用
                    class_name = win32gui.GetClassName(self.master_hwnd)
                    print(f"ウィンドウクラス: {class_name}")
                    
                    if "WindowsForms" in class_name:
                        print("WinFormsアプリケーション検出 - まず従来のSetParent方式を試行")
                        # WinFormsでも一旦SetParentを試してみる
                        print(f"SetParentを実行: {self.master_hwnd} -> {parent_hwnd}")
                        result = win32gui.SetParent(self.master_hwnd, parent_hwnd)
                        print(f"SetParent結果: {result} ({'OK' if result != 0 else 'FAILED'})")
                        
                        if result == 0:
                            print("SetParentが失敗 - 位置同期方式にフォールバック")
                            self._use_position_sync = True
                            self._setup_position_sync()
                            return
                        # SetParentが成功した場合は通常処理を続行
                    else:
                        # 通常のアプリの場合は従来のSetParent方式
                        print(f"SetParentを実行: {self.master_hwnd} -> {parent_hwnd}")
                        result = win32gui.SetParent(self.master_hwnd, parent_hwnd)
                        print(f"SetParent結果: {result} ({'OK' if result != 0 else 'FAILED'})")
                        
                        if result == 0:
                            print("SetParentが失敗しました")
                            return
    ...
    def _setup_position_sync(self):
        """WinFormsアプリ用の位置同期システムをセットアップ"""
        try:
            # ウィンドウスタイルを調整（枠線を削除）
            style = win32gui.GetWindowLong(self.master_hwnd, win32con.GWL_STYLE)
            # タイトルバー、枠線、リサイズ境界を削除
            ...
            # 拡張スタイルも調整
            ex_style = win32gui.GetWindowLong(self.master_hwnd, win32con.GWL_EXSTYLE)
            ex_style = ex_style & ~(win32con.WS_EX_DLGMODALFRAME | win32con.WS_EX_CLIENTEDGE | win32con.WS_EX_STATICEDGE)
            win32gui.SetWindowLong(self.master_hwnd, win32con.GWL_EXSTYLE, ex_style)
            
            # スタイル変更を適用
            win32gui.SetWindowPos(self.master_hwnd, 0, 0, 0, 0, 0, 
            ...
            # 位置同期タイマーを開始
            if not hasattr(self, 'position_sync_timer'):
                self.position_sync_timer = QTimer()
                self.position_sync_timer.timeout.connect(self._sync_window_position)
            self.position_sync_timer.start(100)  # 100msごとに位置を同期
            
            # 埋め込みタイマーを停止
            self.embed_timer.stop()
            
            print("WinFormsアプリ用位置同期システムを開始しました")
            
        except Exception as e:
            print(f"位置同期セットアップエラー: {str(e)}")
    
    def _sync_window_position(self):
        """ウィンドウ位置をウィジェットに同期"""
        try:
            if not self.master_hwnd or not hasattr(self, '_use_position_sync'):
                return
                
            # タブが変更された場合の処理
            current_tab = self.tabs.currentIndex()
            if current_tab != 1:  # マスタタブ以外の場合
                win32gui.ShowWindow(self.master_hwnd, win32con.SW_HIDE)
                return
            else:
                win32gui.ShowWindow(self.master_hwnd, win32con.SW_SHOW)
                
            # ウィジェットのグローバル座標を取得
            widget_global_pos = self.master_embed_widget.mapToGlobal(self.master_embed_widget.rect().topLeft())
            widget_size = self.master_embed_widget.size()
            
            print(f"位置同期: ウィジェット位置=({widget_global_pos.x()}, {widget_global_pos.y()}), サイズ={widget_size.width()}x{widget_size.height()}")
            
            # ボーダー幅を考慮したサイズ調整
            border_width = 4
            adjusted_width = max(widget_size.width() - border_width, 50)
            adjusted_height = max(widget_size.height() - border_width, 50)
            
            # ウィンドウを移動・リサイズ
            result = win32gui.SetWindowPos(
                self.master_hwnd,
                win32con.HWND_TOP,  # TOPMOSTではなくTOPに変更
                widget_global_pos.x() + 2,  # ボーダー分の調整
                widget_global_pos.y() + 2,
                adjusted_width,
                adjusted_height,
                win32con.SWP_SHOWWINDOW | win32con.SWP_NOACTIVATE
            )
            
            print(f"SetWindowPos結果: {result}, 調整後サイズ: {adjusted_width}x{adjusted_height}")
            
        except Exception as e:
            print(f"位置同期エラー: {str(e)}")
    ...
    def resize_embedded_window(self):
        """埋め込まれたウィンドウのサイズを調整"""
        print(f"resize_embedded_window呼び出し - master_hwnd: {self.master_hwnd}")
        if not self.master_hwnd:
            print("ウィンドウハンドルがないためリサイズをスキップ")
            return
            
        # 位置同期方式の場合は、同期処理に任せる
        if hasattr(self, '_use_position_sync') and self._use_position_sync:
            print("位置同期方式使用中のため、resize処理をスキップ")
            return
            
        try:
            parent_rect = self.master_embed_widget.geometry()
            current_size = (parent_rect.width(), parent_rect.height())
            print(f"現在の埋め込みウィジェットサイズ: {current_size}")
            
            # サイズが小さすぎる場合はスキップ
            if current_size[0] < 100 or current_size[1] < 100:
                print("サイズが小さすぎるためスキップ")
                return
            
            # 前回と同じサイズの場合はスキップ（重複実行防止）
            if hasattr(self, 'last_resize_size') and self.last_resize_size == current_size:
                print("前回と同じサイズのため処理をスキップ")
                return
            
            # ウィンドウが最小化されていないかチェック
            is_visible = win32gui.IsWindowVisible(self.master_hwnd)
            print(f"対象ウィンドウの可視性: {is_visible}")
            if not is_visible:
                print("対象ウィンドウが非表示のためリサイズをスキップ")
                return
            ...
            print(f"調整後のサイズ: {adjusted_width}x{adjusted_height}")
            
            print(f"SetWindowPos実行: HWND={self.master_hwnd}, Size={adjusted_width}x{adjusted_height}")
            result = win32gui.SetWindowPos(
                self.master_hwnd, 
                win32con.HWND_TOP,
            ...
            print(f"SetWindowPos結果: {result} ({'成功' if result != 0 else '失敗'})")
            
            # 表示の問題を解決するための追加処理
            try:
                # 1. ウィンドウを一旦隠して再表示（強制リフレッシュ）
                win32gui.ShowWindow(self.master_hwnd, win32con.SW_HIDE)
                QApplication.processEvents()
                time.sleep(0.1)  # 短い待機
                win32gui.ShowWindow(self.master_hwnd, win32con.SW_SHOW)
                ...
                # 2. ウィンドウを強制的に最前面に表示
                win32gui.BringWindowToTop(self.master_hwnd)
                
                # 3. SetWindowPosで再度位置・サイズを確実に設定
                win32gui.SetWindowPos(
                    self.master_hwnd,
                    win32con.HWND_TOP,
                    0, 0,
                ...
                try:
                    widget_global_pos = self.master_embed_widget.mapToGlobal(self.master_embed_widget.rect().topLeft())
                    print(f"絶対座標でのウィンドウ配置を試行: ({widget_global_pos.x()}, {widget_global_pos.y()})")
                    win32gui.SetWindowPos(
                        self.master_hwnd,
                        win32con.HWND_TOP,
                        widget_global_pos.x(), widget_global_pos.y(),
                        adjusted_width, adjusted_height,
                        win32con.SWP_SHOWWINDOW | win32con.SWP_FRAMECHANGED
                    )
                except Exception as abs_pos_error:
                    print(f"絶対座標配置エラー: {abs_pos_error}")
                
                # 4. ウィンドウの再描画を強制
                win32gui.InvalidateRect(self.master_hwnd, None, True)
                win32gui.UpdateWindow(self.master_hwnd)
                
                # 5. 親ウィジェットも更新
                self.master_embed_widget.update()
                self.master_embed_widget.repaint()
                
                # 6. WinFormsアプリ特有の処理
                try:
                    # WinFormsアプリの場合、WM_SIZEメッセージを送信
                    import win32con
                    win32gui.SendMessage(self.master_hwnd, win32con.WM_SIZE, 0, 
                                       (adjusted_height << 16) | adjusted_width)
                    
                    # フォーカスを設定してアクティブにする
                    win32gui.SetFocus(self.master_hwnd)
                    
                    # 強制的にペイントメッセージを送信
                    win32gui.SendMessage(self.master_hwnd, win32con.WM_PAINT, 0, 0)
                    
                    print("WinFormsアプリ特有の処理を実行しました")
                except Exception as winforms_error:
                    print(f"WinForms処理エラー: {winforms_error}")
                
                print("強化された表示処理を実行しました")
            except Exception as display_error:
                print(f"表示処理エラー: {display_error}")
            
            # 実際のウィンドウサイズとスタイルを確認
            try:
                rect = win32gui.GetWindowRect(self.master_hwnd)
                actual_width = rect[2] - rect[0]
                actual_height = rect[3] - rect[1]
                print(f"SetWindowPos後の実際のウィンドウサイズ: {actual_width}x{actual_height}")
                
                # ウィンドウスタイルも確認
                style = win32gui.GetWindowLong(self.master_hwnd, win32con.GWL_STYLE)
                ex_style = win32gui.GetWindowLong(self.master_hwnd, win32con.GWL_EXSTYLE)
                print(f"ウィンドウスタイル: 0x{style:08x}, 拡張スタイル: 0x{ex_style:08x}")
                ...
                print(f"スタイル詳細: Caption={has_caption}, Border={has_border}, ThickFrame={has_thickframe}, Child={is_child}")
                
                # ウィンドウクラス名も確認
                try:
                    class_name = win32gui.GetClassName(self.master_hwnd)
                    print(f"ウィンドウクラス名: {class_name}")
                    
                    # ウィンドウの座標位置も確認
                    client_rect = win32gui.GetClientRect(self.master_hwnd)
                    window_rect = win32gui.GetWindowRect(self.master_hwnd)
                    print(f"クライアント領域: {client_rect}")
                    print(f"ウィンドウ領域: {window_rect}")
                    
                    # 埋め込みウィジェットの絶対座標
                    widget_global_pos = self.master_embed_widget.mapToGlobal(self.master_embed_widget.rect().topLeft())
                    widget_global_rect = (widget_global_pos.x(), widget_global_pos.y(), 
                                        widget_global_pos.x() + self.master_embed_widget.width(),
                                        widget_global_pos.y() + self.master_embed_widget.height())
                    print(f"埋め込みウィジェットのグローバル座標: {widget_global_rect}")
                    
                except Exception as coord_error:
                    print(f"座標確認エラー: {coord_error}")
                
                # 親ウィンドウ関係も確認
                try:
                    parent_hwnd = win32gui.GetParent(self.master_hwnd)
                    expected_parent = int(self.master_embed_widget.winId())
                    print(f"実際の親HWND: {parent_hwnd}, 期待する親HWND: {expected_parent}")
                    print(f"親子関係: {'正常' if parent_hwnd == expected_parent else '異常'}")
                except:
                    pass
                
            except Exception as size_check_error:
                print(f"実際のサイズ・スタイル取得エラー: {size_check_error}")
